...
def get_menu_sorted_with_vote(date):
    ...
    """
    return (
        Menu.objects.filter(date=date)
        .annotate(vote_count=Count("vote"))
        .order_by("-vote_count", "created_at")
    )


@extend_schema_view(post=extend_schema(summary="Vote for menu"))
...
@extend_schema_view(get=extend_schema(summary="Select winner"))
class WinnerRetrieveApiView(RetrieveAPIView):
    ...
    def get_object(self):
        ...
        yesterday_winner = get_menu_sorted_with_vote(
            today + datetime.timedelta(days=-1)
        ).first()
        day_before_yesterday_winner = get_menu_sorted_with_vote(
        ...
        if (
            yesterday_winner.restaurant_id == day_before_yesterday_winner.restaurant_id
            and day_before_yesterday_winner.restaurant_id
            == today_menu_list.first().restaurant_id
        ):
